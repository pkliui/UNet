...
class ResizeData(nn.Module):
    ...
    def __call__(self, orig_sample: dict) -> dict:
        ...
        resized_sample = {'image': self.resizer(orig_sample['image'], self.new_image_size),
                          'mask': self.resizer(orig_sample['mask'], self.new_mask_size)}
        #
    ...
    def resizer(self, original_image, new_size):
        """
    ...
    def _validate_input_size(self, original_image, original_mask):
        ...
        if len(original_image) == 3 and original_image.shape[-3] == 3:
        ...
        elif len(original_image) == 3 and original_image.shape[-1] == 3:
        ...
        if len(original_mask) == 3 and original_mask.shape[-3] == 1:
        ...
        elif len(original_mask) == 3 and original_mask.shape[-1] == 1:
